...
export default function ProjectDashboardPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d0e12] text-slate-200 overflow-hidden relative font-sans">
        ...
        <div className="w-72 flex flex-col border-r border-white/5 bg-black/40">
           <div className="p-4">
             <div className="relative group">
               ...
               <Input placeholder="Buscar en ClickUp..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 h-9 bg-white/5 border-white/10 rounded-lg text-xs" />
             </div>
           </div>
           <div className="flex-1 overflow-y-auto px-2 space-y-1 custom-scrollbar">
              {workspaces.map(ws => (
                <div key={ws.id}>
                   <div className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${selectedWorkspaceId === ws.id && !selectedFolderId ? "bg-primary/10 text-primary" : "hover:bg-white/5"}`} onClick={() => {
