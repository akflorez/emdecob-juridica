...
function TaskList({ tasks, isLoading, onTaskClick, isUrgent }: TaskListProps) {
  ...
  return (
    <div className="bg-card/10 rounded-[3rem] border border-border/50 overflow-hidden shadow-2xl">
       <div className="grid grid-cols-[1fr_180px_120px_180px] gap-10 px-12 py-6 border-b border-border/50 text-[9px] font-black uppercase text-muted-foreground tracking-[0.4em] bg-muted/20">
       ...
       <div className="divide-y divide-border/50">
          {tasks.map(task => (
            <div 
               key={task.id} 
               className="grid grid-cols-[1fr_180px_120px_180px] gap-10 px-12 py-5 hover:bg-muted/30 transition-all cursor-pointer group animate-in slide-in-from-bottom-3 duration-500 border-l-2 border-transparent hover:border-primary"
               onClick={() => onTaskClick(task)}
            >
               <div className="flex items-center gap-6">
                  <div className="h-10 w-10 rounded-xl bg-background border border-border/50 flex items-center justify-center text-primary shadow-xl group-hover:scale-110 group-hover:border-primary/40 transition-all">
                     <Hash className="h-5 w-5" />
                  </div>
                  <div className="flex flex-col gap-1">
                     <span className="text-[15px] font-black text-foreground group-hover:text-primary transition-colors tracking-tight leading-tight">{task.title}</span>
                     <span className="text-[10px] text-muted-foreground font-black tracking-widest uppercase opacity-60">REF: {task.clickup_id || task.id}</span>
                  </div>
               </div>
