...
def full_sync_to_contabo():
    ...
    print("Connecting to Contabo Juricob...")
    ...
    for ws in db_local.query(Workspace).all():
        if not db_prod.query(Workspace).filter(Workspace.clickup_id == ws.clickup_id).first():
            new_ws = Workspace(id=ws.id, name=ws.name, owner_id=ws.owner_id, clickup_id=ws.clickup_id)
            db_prod.add(new_ws)
    db_prod.commit()
    ...
    for ev in db_local.query(CaseEvent).all():
        # Check if Case exists in Prod
        if db_prod.query(Case).filter(Case.id == ev.case_id).first():
    ...
    print("Success: Full Sync to Contabo Complete!")
    db_local.close()
    db_prod.close()
